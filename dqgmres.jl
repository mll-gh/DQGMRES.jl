...
mutable struct DQGMRESIterable{preclT, precrT, solT, rhsT, vecT, mem <: Int, arnoldiT <: ArnoldiDecomp, residualT <: Residual, resT <: Real}
    Pl::preclT
    Pr::precrT
    x::solT
    b::rhsT
...
converged(g::DQGMRESIterable) = g.residual.current ≤ g.reltol

start(::DQGMRESIterable) = 0

done(g::DQGMRESIterable, iteration::Int) = iteration ≥ g.maxiter || converged(g)

function next(g::DQGMRESIterable, iteration::Int)
    ...
    g.arnoldi.H[g.k + 1, g.k] = orthogonalize_and_normalize!(
        view(g.arnoldi.V, :, max(1, g.k - g.z + 1) : g.k),
...
function dqgmres_iterable!(x, A, b, z;
    Pl = Identity(),
    Pr = Identity(),
    ...
    DQGMRESIterable(Pl, Pr, x, b, Ax, z,
        arnoldi, residual,
...
"""
    dqgmres(A, b, z; kwargs...) -> x, [history]

Same as [`dqgmres!`](@ref), but allocates a solution vector `x` initialized with zeros.
"""
dqgmres(A, b, z; kwargs...) = dqgmres!(zerox(A, b), A, b, z; initially_zero = true, kwargs...)

"""
    dqgmres!(x, A, b, z; kwargs...) -> x, [history]

Solves the problem ``Ax = b`` with restarted DQGMRES.
...
- `restart::Int = min(20, size(A, 2))`: restarts DQGMRES after specified number of iterations;
- `maxiter::Int = size(A, 2)`: maximum number of inner iterations of DQGMRES;
...
function dqgmres!(x, A, b, z;
  ...
  verbose::Bool = false
    ...
    iterable = dqgmres_iterable!(x, A, b, z; Pl = Pl, Pr = Pr, tol = tol, maxiter = maxiter, restart = restart, initially_zero = initially_zero)

    verbose && @printf("=== dqgmres ===\n%4s\t%4s\t%7s\n","rest","iter","resnorm")
